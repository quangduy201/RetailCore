...
import { BrandStatusBadge } from "../common/StatusBadge";
import {
  BRAND_TABLE_COLUMNS,
  type BrandColumnKey,
} from "../../constants/brandTableColumns";
...
interface BrandTableProps {
  readonly brands: BrandDetailDto[];
  ...
  readonly onEdit: (brandId: string) => void;
  readonly onDelete: (brandId: string) => void;
}
  ...
  return (
    <Table variant="primary" className="border border-default">
      <Table.ResizableContainer>
        ...
        >
          <Table.Header columns={BRAND_TABLE_COLUMNS}>
            {(column) => (
              <Table.Column
                id={column.key}
      ...
      <Table.Footer className="border-t border-default-200 px-6 py-4">
        ...
        <Pagination className="flex w-full flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <Pagination.Summary className="text-sm text-default-500">
            Showing {showingFrom} to {showingTo} of {totalCount} brands
          </Pagination.Summary>
